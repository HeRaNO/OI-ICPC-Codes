...
int main()
{
    ...
    n=read();T=read();
    for (int i=1;i<=n;i++) {a[i].first=read();a[i].second=&b[i];}
    sort(a+1,a+n+1);
    for (int i=1;i<=n;i++)
    {
    ...
    for (int i=1;i<=n;i++) Monotonous_Stack((stack){b[i],i,i});
    Monotonous_Stack((stack){INT_MAX,n+1,n+1});
    for (int i=1;i<=m;i++) cnt[i]+=cnt[i-1];
    while (T--)
    {
        scanf("%s",opt);x=read();r=x;x=binary(x);
        if (opt[0]=='=')
        {
            if (ori[x]!=r) puts("0");
            else printf("%lld\n",cnt[x]-cnt[x-1]);
