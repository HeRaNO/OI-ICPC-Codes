...
int main()
{
    ...
    add(root,make_pair(1e18,0));add(root,make_pair(-1e18,0));
    for (int i=n;i;i--)
    {
        pii pre,aft,now;int k;
        now=make_pair(a[i],i);
        add(root,now);
        pre=prec(root,now);aft=succ(root,now);
        if (i==n) continue;
        if (i==n-1) {bnxt[i]=n;continue;}
        if (myabs(pre.first-a[i])<=myabs(aft.first-a[i]))
        {
            bnxt[i]=pre.second;now=prec(root,pre);
            k=(myabs(now.first-a[i])<=myabs(aft.first-a[i])?now.second:aft.second);
        }
        else
        {
            bnxt[i]=aft.second;now=succ(root,aft);
    ...
    {
        arrive=pos;rest_dis=x0;best_a=best_b=0;
        for (int i=17;i;i--)
            if (father[arrive][i]&&dis_a[arrive][i]+dis_b[arrive][i]<=rest_dis)
            {
                best_a+=dis_a[arrive][i];best_b+=dis_b[arrive][i];
    ...
    for (int j=1;j<=m;j++)
    {
        best_a=best_b=0;
        for (int i=17;i;i--)
            if (father[s[j]][i]&&dis_a[s[j]][i]+dis_b[s[j]][i]<=x[j])
            {
                best_a+=dis_a[s[j]][i];best_b+=dis_b[s[j]][i];
                x[j]-=dis_a[s[j]][i]+dis_b[s[j]][i];
                s[j]=father[s[j]][i];
            }
        if (dis_a[s[j]][1]<=x[j]) best_a+=dis_a[s[j]][1];
        printf("%lld %lld\n",best_a,best_b);
    }
    return 0;
}
