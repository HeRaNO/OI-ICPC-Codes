...
void dfs(int x,int y)
{
	...
	if (x==4)
	{
		...
		if (ax!=4)
		{
			...
			for (int i=1;i<=4;i++)
			{
				if (used[a[x][i]]) {no=true;pos=i;break;}
		...
		else
		{
			...
			if (!no)
			{
				...
				if (a[1][4]+a[2][3]+a[3][2]+a[4][1]!=34) no=true;
				if (!no) print();
				for (int i=1;i<=4;i++) used[a[x][i]]=false;used[1]=true;
			}
			else for (int i=1;i<pos;i++) used[a[x][i]]=false;used[1]=true;
		}
		return ;
	}
