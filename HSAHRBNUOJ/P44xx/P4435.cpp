...
{
	if (tree[u << 1].stop && tree[u << 1 | 1].stop) tree[u].stop = true;
...
{
	if (tree[u].stop) return ;
	if (tree[u].p + 1 == tree[u].r)
	{
		if ((++tree[u].updcnt) == cnt) tree[u].stop = true;
