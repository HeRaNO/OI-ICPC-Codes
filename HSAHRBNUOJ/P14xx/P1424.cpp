...
int main()
{
	...
	for (int i = 1; i <= m; i++)
		for (int j = 1; j <= n; j++)
		{
			read(x);
			sum += x;
			if ((i & 1) ^ (j & 1)) add(S, GetPos(i, j), x);
			else add(GetPos(i, j), T, x);
			for (int k = 0; k < 4; k++)
			{
				int tx = i + d[k][0], ty = j + d[k][1];
				if (tx < 1 || tx > m || ty < 1 || ty > n) continue;
				if ((i & 1) ^ (j & 1)) add(GetPos(i, j), GetPos(tx, ty), ~(1 << 31));
				else add(GetPos(tx, ty), GetPos(i, j), ~(1 << 31));
			}
		}
