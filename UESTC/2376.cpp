...
int main()
{
	read(n);p[0]={1<<31,NULL};
	for (int i=1,x;i<=n;i++) read(a[i]),p[i]={a[i],&a[i]};
	sort(p+1,p+n+1);
	for (int i=1;i<=n;i++)
	{
	...
	for (int i=1;i<=n;i++)
	{
		modify(1,a[i],i);int x=query(a[i]);
		if (x) changemin(1,1,a[i],x);
		if (a[i]<n) add(1,a[i]+1,n+1,1);
		add(a[i]);ans[i]+=tree[1].x;a[i]=n-a[i]+1;
	}
